...
class Home_model extends CI_Model {
    ...
    public function getUserListings($id){
        $this->db->select('l.*,(select image_name from listing_images where l.id = listing_images.listing_id order by sort_order asc limit 1 ) as image_name, pt.name as property_type');
        $this->db->from('listings l');
        $this->db->join('property_types pt', 'pt.id = l.property_type', 'left');
